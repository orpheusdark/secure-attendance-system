  ...
  return (
    <main className="space-y-8">
      <SectionCard title="Attendance records" eyebrow="Live data grid">
        <div className="overflow-hidden rounded-[26px] border border-white/10 bg-slate-950/50">
          <div className="grid grid-cols-[1.2fr_1fr_0.7fr_0.6fr] gap-4 border-b border-white/10 px-5 py-4 text-xs uppercase tracking-[0.35em] text-slate-500">
            <span>Name</span><span>Device trust</span><span>Confidence</span><span>Fraud</span>
          </div>
          <div className="divide-y divide-white/10">
            {rows.map((row) => (
              <div key={row.name} className="grid grid-cols-[1.2fr_1fr_0.7fr_0.6fr] gap-4 px-5 py-4 text-sm text-slate-300">
